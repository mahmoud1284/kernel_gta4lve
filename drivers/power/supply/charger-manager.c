...
/**
 *
 * get_batt_cycle - Get the cycle of the battery
 * @cm: the Charger Manager representing the battery.
 * @batt_cycle: the battery cycle returned.
 *
 * Returns 0 if there is no error.
 * Returns a negative value on error.
 */
static int get_batt_cycle(struct charger_manager *cm, int *batt_cycle)
{
	union power_supply_propval val;
	struct power_supply *fuel_gauge;
	int ret;
...
//+bug696110,tankaikun.wt,add,2022/08/08,add bat protect
static int cm_manager_batt_cycle_cv_monitor(struct charger_manager *cm) {
	int i,cycle;
	int target_cccv=0;
	int target_full_vol=0;
	int batt_cycle=0;
	int ret = 0;
	int batt_uV = 0;

	ret = get_batt_cycle(cm, &batt_cycle);
	if (ret)
		dev_err(cm->dev, "get batt_cycle error.\n");
	...
	if(batt_cycle >= 0 && batt_cycle < 999999)
		cycle = batt_cycle;
	else
		return -EINVAL;

	if (cm->desc->batt_cycle_fv_cfg && cm->desc->fv_levels) {
		for (i = 0; i < cm->desc->fv_levels; i=i+4) {
			if ((cycle >= cm->desc->batt_cycle_fv_cfg[i]) && (cycle <= cm->desc->batt_cycle_fv_cfg[i+1])) {
				target_cccv = cm->desc->batt_cycle_fv_cfg[i+2];
				target_full_vol = cm->desc->batt_cycle_fv_cfg[i+3];
				if (!cm->desc->cm_charge_vote_cv_flag || cm->desc->batt_cycle_cv!=target_cccv) {
					if (batt_uV >= target_cccv) {
						if (cm->desc->batt_cycle_trigger_cnt<2)
							cm->desc->batt_cycle_trigger_cnt++;
						dev_err(cm->dev, "batt_uV >= target_cccv cnt=%d batt_uV=%d full_vol=%d\n",
									cm->desc->batt_cycle_trigger_cnt, batt_uV, target_cccv);
						return 0;
					} else {
						if (cm->desc->batt_cycle_trigger_cnt > 0)
							cm->desc->batt_cycle_trigger_cnt--;
					}

					if (cm->desc->batt_cycle_trigger_cnt > 0) {
						dev_err(cm->dev, "batt_uV >= target_cccv cnt=%d \n", cm->desc->batt_cycle_trigger_cnt);
						return 0;
					}

					cm->cm_charge_vote->vote(cm->cm_charge_vote, true,
								SPRD_VOTE_TYPE_CCCV,
								SPRD_VOTE_TYPE_CCCV_ID_BATT_CYCLE,
								SPRD_VOTE_CMD_MIN,
								target_cccv, cm);
					cm->desc->cm_charge_vote_cv_flag = true;
					cm->desc->batt_cycle_cv = target_cccv;
					cm->desc->fullbatt_uV = target_full_vol;
					cm->desc->batt_cycle_trigger_cnt = 0;
					dev_err(cm->dev, "set batt cycle cv = %d\n", target_cccv);
				}
				return 0;
			}
		}
	} else {
		target_cccv = cm->desc->cm_charge_target_cccv;
		cm->cm_charge_vote->vote(cm->cm_charge_vote, false,
					SPRD_VOTE_TYPE_CCCV,
					SPRD_VOTE_TYPE_CCCV_ID_BATT_CYCLE,
					SPRD_VOTE_CMD_MIN,
					target_cccv, cm);
		cm->desc->cm_charge_vote_cv_flag = false;
	}
	return 0;
}
//-bug696110,tankaikun.wt,add,2022/08/08,add bat protect
...
static int cm_get_target_status(struct charger_manager *cm)
{
	...
	//+bug696110,tankaikun.wt,add,2022/08/08,add bat protect
	ret = cm_manager_batt_cycle_cv_monitor(cm);
	if (ret)
		printc("Errors orrurs when adjusting charging voltage\n");
	//-bug696110,tankaikun.wt,add,2022/08/08,add bat protect
...
static void cm_batt_full_capacity_check(struct charger_manager *cm)
{
        ...
        if(batt_full_capacity < 100 && (is_ext_pwr_online(cm))){
                ...
                if(fuel_cap >= batt_full_capacity && !(cm->wt_charging_status & WT_CHARGE_FULL_CAP_DISCHG)){
                        ...
                        power_supply_changed(cm->charger_psy);
                        printb("cm_batt_full_capacity_check: cap = %d, WT_CHARGE_FULL_CAP_DISCHG!\n", fuel_cap);
                }
                if(fuel_cap <= (batt_full_capacity - 2) && (cm->wt_charging_status & WT_CHARGE_FULL_CAP_DISCHG)){
                        ...
                        if(!(cm->wt_charging_status & ~WT_CHARGE_FULL_CAP_DISCHG)){
                                printb("cm_batt_full_capacity_check:cap = %d, restore charge!\n", fuel_cap);
                                cm->wt_charger_enabled = true;
                                try_charger_enable(cm, true);
                                cm->desc->charger_status = cm->battery_status = POWER_SUPPLY_STATUS_CHARGING;
                                power_supply_changed(cm->charger_psy);
                        }else{
                                printb("cm_batt_full_capacity_check: Other ctrl discharge(0x%x)!\n",cm->wt_charging_status);
                        }
                }
        }
				...
				printb("cm_batt_full_capacity_check : is_ext_pwr_online=0");
			}
                }
        }
}
...
static void misc_event_handler(struct charger_manager *cm, enum cm_event_types type)
{
	...
	if (is_ext_pwr_online(cm)) {
		...
		cm->desc->cm_charge_vote_cv_flag = false; //bug696110,tankaikun.wt,add,2022/08/08,add bat protect
		//cm->desc->bat_ovp_flag = 0;
			    ...
			    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_AFC_FLAG:
		...
		break;
	//- add usb sys node by guoyanjun
	//+bug798866,add voltage_now in usb
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		get_charger_voltage(wt_cm, &val->intval);
		break;
	//-bug798866,add voltage_now in usb
	default:
		ret = -EINVAL;
...
static enum power_supply_property usb_props[] = {
	...
	POWER_SUPPLY_PROP_VOLTAGE_NOW, //bug798866,add  voltage_now in usb
	//- add usb sys node by guoyanjun
};
...
static enum power_supply_property default_charger_props[] = {
	...
	POWER_SUPPLY_PROP_CYCLE_COUNT, //bug696110,tankaikun.wt,add,2022/08/08,add bat protect
	/*
...
static struct charger_desc *of_cm_parse_desc(struct device *dev)
{
	...
	//+bug696110,tankaikun.wt,add,2022/08/08,add bat protect
	of_property_read_u32(np, "wt,float-voltage-mv",
					&desc->cm_charge_target_cccv);

	desc->batt_cycle = 0;
	if (of_find_property(np, "wt,batt-cycle-ranges", &byte_len)) {
		desc->batt_cycle_fv_cfg = devm_kzalloc(dev, byte_len, GFP_KERNEL);
		if(desc->batt_cycle_fv_cfg) {
			desc->fv_levels = byte_len / sizeof(u32);
			rc = of_property_read_u32_array(np,
				"wt,batt-cycle-ranges",
				desc->batt_cycle_fv_cfg,
				desc->fv_levels);
			if (rc < 0) {
				dev_err(dev,
					"Couldn't read battery protect limits rc = %d\n", rc);
				desc->batt_cycle_fv_cfg = NULL;
			}
		}
	}
	//-bug696110,tankaikun.wt,add,2022/08/08,add bat protect
