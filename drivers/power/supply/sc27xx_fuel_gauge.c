...
#define printc(fmt, args...) do { printk("[wt_usb][fgu][%s] "fmt, __FUNCTION__, ##args); } while (0)
...
//+bug696110,tankaikun.wt,add,2022/08/08,add bat protect
#define SC27XX_FGU_CAP_MAGIC_NUM		0x5A5AA5A5
#define SC27XX_FGU_DATA_FILE_PATH 		"/dev/block/by-name/miscdata"

static const int batt_info_check_cycle[] = {
	300, 400, 700, 1000
};
//-bug696110,tankaikun.wt,add,2022/08/08,add bat protect
...
struct sc27xx_fgu_data {
	...
	u32 chg_type;
	//+bug696110,tankaikun.wt,add,2022/08/08,add bat protect
	u32 charge_cycle_cnt;
	u32 charge_accumulate_cnt;
	int file_offset;
	int file_size;
	u32 file_buf[8];
	bool read_file;
	...
	struct delayed_work file_data_work;
...
{
	//+bug773210,wujishuai.wt,20220701,correct the soc if cap is wrong
	int temp_real_cap, temp_cap_cal, vbat_now, flag = 1;
	int vbat_ocv, vat_ocv_cap, vbat_ocv_flag = 0, vbar_ocv_cap_diff;
	//-bug773210,wujishuai.wt,20220701,correct the soc if cap is wrong
	...
	if (!is_first_poweron) {
		...
		//+bug773210,wujishuai.wt,20220701,correct the soc if cap is wrong
		/*
		*temperature must be obtained in advance
		*because the resiatance value needs to be calculated according to temperature on sc27xx_fgu_get_vbat_ocv
		*/
		...
		sc27xx_fgu_get_boot_voltage(data, &ocv);  //pocv
		/*
		*It is neccessary to determine whether the pocv is corrent.
		*Because the pocv will not change when reboot
		*/
		sc27xx_fgu_get_vbat_ocv(data, &vbat_ocv);
		sc27xx_fgu_get_vbat_now(data, &vbat_now);
		vbat_now *= 1000;
		dev_info(data->dev, " sc27xx_fgu_get_vbat_now is %d, the sc27xx_fgu_get_vbat_ocv is %d \n", vbat_now, vbat_ocv);
		dev_info(data->dev, " the abs ocv - vbat_ocv is %d \n", abs(ocv - vbat_ocv));

		temp_real_cap  = power_supply_ocv2cap_simple(data->cap_table, data->table_len, ocv);
		temp_real_cap *= 10;
		vat_ocv_cap = power_supply_ocv2cap_simple(data->cap_table, data->table_len, vbat_ocv);
		vat_ocv_cap *= 10;
		dev_info(data->dev, " vat_ocv_cap is %d, temp_real_cap is %d, abs(vat_ocv_cap - temp_real_cap) is %d, abs(*cap - temp_real_cap) is %d \n",
			vat_ocv_cap, temp_real_cap, abs(vat_ocv_cap - temp_real_cap), abs(*cap - temp_real_cap));

		if (abs(ocv - vbat_ocv) > 50000) {
		/*
		*if the pocv and vbat values are to different
		*maybe the pocv is not change because the phone is rebooting
		*so it will not re-cal the cap when reboot
		*it only run re-cal cap when the phone shutdown and power on
		*but if add re-cal-cap when reboot in specific time
		*the function will more fully
		*/
			flag = 0;
			if(abs(*cap - vat_ocv_cap) > 250) {
				vbat_ocv_flag = 1;
				flag = 1;
			}
		}

		temp_cap_cal = abs(temp_real_cap - *cap);
		vbar_ocv_cap_diff = abs(*cap - vat_ocv_cap);

		dev_info(data->dev, "temp_cap_cal = %d, vbar_ocv_cap_diff = %d, temp_cap = %d, normal_cap = %d, tempaerature is = %d, flag is %d, vbat_ocv_flag is %d\n",
			temp_cap_cal, vbar_ocv_cap_diff, temp_real_cap, *cap, data->bat_temp, flag, vbat_ocv_flag);

		if (((temp_cap_cal >= 250) || (vbar_ocv_cap_diff >= 250)) && ((data->bat_temp > 100) && (data->bat_temp < 450)) && (flag == 1)) {
			dev_info(data->dev, "the difference value is %d\n", temp_cap_cal);
		} else {
			dev_info(data->dev, "sc27xx_fgu_save_boot_mode\n");
	...
	//+bug773210,wujishuai.wt,20220701,correct the soc if cap is wrong
	if (vbat_ocv_flag == 1) {
		sc27xx_fgu_get_vbat_ocv(data, &ocv);
		dev_info(data->dev, " the use vbat ocv\n");
	} else {
		sc27xx_fgu_get_boot_voltage(data, &ocv);
		dev_info(data->dev, " the use pocv \n");
	}
	//+bug773210,wujishuai.wt,20220701,correct the soc if cap is wrong
...
static int data_average_method(int data)
{
	...
	printk("emdoor-%s:data=%d, avgdata=%d, Index=%d\n",__func__,data,avgdata,Index);
	return avgdata;
}

//+bug696110,tankaikun.wt,add,2022/08/08,add bat protect
static void sc27xx_fgu_file_data_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct sc27xx_fgu_data *data = container_of(dwork,
			struct sc27xx_fgu_data, file_data_work);
	struct file *filep;
	static int retry_cnt = 5;
	u32 file_buf[4];
	loff_t pos = (10 * 1024 + 256);
	int ret;

	mutex_lock(&data->file_lock);
	filep = filp_open(SC27XX_FGU_DATA_FILE_PATH, O_RDWR, 0);
	if (IS_ERR(filep)) {
		dev_err(data->dev, "failed to open file:/dev/block/by-name/miscdata, ret = %d.\n", PTR_ERR(filep));
		if (retry_cnt > 0) {
			retry_cnt--;
			schedule_delayed_work(&data->file_data_work, msecs_to_jiffies(5000));
		}
		goto out;
	}
	retry_cnt = 5;
	...
	if (data->read_file) {
		ret = kernel_read(filep, (char *)&file_buf, sizeof(file_buf), &pos);
		if (ret < 0) {
			dev_err(data->dev, "/dev/block/by-name/miscdata file is empty or read error, ret = %d\n", ret);
			schedule_delayed_work(&data->file_data_work, msecs_to_jiffies(500));
			goto out;
		}
	...
	if (data->write_file) {
		pos = (10 * 1024 + 256 + data->file_offset);
		ret = kernel_write(filep, &data->file_buf, data->file_size, &pos);
		if (ret < 0) {
			dev_err(data->dev, "write file_buf data error, ret = %d\n", ret);
			schedule_delayed_work(&data->file_data_work, msecs_to_jiffies(500));
			goto out;
		}
		data->write_file = false;
		dev_info(data->dev, "write file_buf data ok, pos = %lld, size = %ld\n", pos, data->file_size);
	}
...
static int sc27xx_fgu_get_capacity(struct sc27xx_fgu_data *data, int *cap,
				   int chg_sts)
{
	int ret, cur_clbcnt, delta_clbcnt, delta_cap, temp, temp_cap, total_mah, avg_temp;
	//+bug696110,tankaikun.wt,add,2022/08/08,add bat protect
	static int last_fgu_cap = SC27XX_FGU_CAP_MAGIC_NUM;
	int fgu_delta_cap;
	//-bug696110,tankaikun.wt,add,2022/08/08,add bat protect
	...
	*cap = delta_cap + data->init_cap;
	...
	if(chg_sts == POWER_SUPPLY_STATUS_DISCHARGING && avg_temp < 100 && avg_temp > 0){
		total_mah -= 2500;
		printk("bms lt -%dmah\n",data->total_mah-total_mah);
	}
	delta_cap = DIV_ROUND_CLOSEST(temp * 1000, total_mah);
	*cap = delta_cap + data->init_cap;

	//+bug696110,tankaikun.wt,add,2022/08/08,add bat protect
	if (last_fgu_cap == SC27XX_FGU_CAP_MAGIC_NUM)
		last_fgu_cap = *cap;
	fgu_delta_cap = *cap - last_fgu_cap;
	dev_err(data->dev, "%s:line%d: fgu_delta_cap = %d, *cap = %d, last_fgu_cap = %d\n",
		__func__, __LINE__, fgu_delta_cap, *cap, last_fgu_cap);
	last_fgu_cap = *cap;

	if (fgu_delta_cap > 0) {
		data->charge_accumulate_cnt += fgu_delta_cap;
		if (data->charge_accumulate_cnt >= 1000) {
			data->charge_accumulate_cnt -= 1000;
			data->charge_cycle_cnt++;
		}
		// save to miscdata
		data->file_buf[0] = data->charge_accumulate_cnt | 0x5A000000;// Add magic number
		data->file_buf[1] = data->charge_cycle_cnt;
		data->file_offset = 4;
		data->file_size = 8;
		data->write_file = true;
		schedule_delayed_work(&data->file_data_work, 0);
	}
	dev_err(data->dev, "%s:line%d: charge_cycle_cnt = %d, charge_accumulate_cnt = %d\n",
			__func__, __LINE__, data->charge_cycle_cnt, data->charge_accumulate_cnt);
	//-bug696110,tankaikun.wt,add,2022/08/08,add bat protect
...
static int sc27xx_fgu_get_bat_id_vol(struct sc27xx_fgu_data *data, int *val)
{
...
static int sc27xx_fgu_get_temp(struct sc27xx_fgu_data *data, int *temp)
{
	...
	//+ Chk 1803,guoyanjun.wt,ADD,2021/12/10,If remove the temperature control,set temp is 250 always.
	#ifdef CONFIG_DISABLE_TEMP_PROTECT
	printc("## Disable temp protect! set temp is 250(<- %d) !\n",*temp);
	*temp = 250;
	#endif
	//- Chk 1803,guoyanjun.wt,ADD,2021/12/10,If remove the temperature control,set temp is 250 always.
				   ...
				   union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BAT_ID_VOLTAGE:
		ret = sc27xx_fgu_get_bat_id_vol(data, &value);
		if (ret)
			goto error;

		val->intval = value;
		printc("bat id voltage = %d\n",value);
		break;
	...
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		val->intval = data->charge_cycle_cnt;
		break;
	//-bug696110,tankaikun.wt,add,2022/08/08,add bat protect
...
//+bug696110,tankaikun.wt,add,2022/08/08,add bat protect
	case POWER_SUPPLY_PROP_CYCLE_COUNT:
		if(val->intval >= 0 && val->intval <= 1000) {
			dev_err(data->dev, "update batt cycle = %d\n", val->intval);
			data->charge_cycle_cnt = val->intval;
			data->charge_accumulate_cnt = 995;
			data->file_buf[0] = data->charge_accumulate_cnt | 0x5A000000;// Add magic number
			data->file_buf[1] = data->charge_cycle_cnt;
			data->file_offset = 4;
			data->file_size = 8;
...
static enum power_supply_property sc27xx_fgu_props[] = {
	...
	POWER_SUPPLY_PROP_BAT_ID_VOLTAGE,
...
/*get basp ocv index from cmdline*/
static int get_cycle_cnt(struct sc27xx_fgu_data *data)
{
	extern char *saved_command_line;
	char result[20] = {};
	char *match = strstr(saved_command_line, "charge_miscdata_migic_number=");
	u32 index = 0;
	int str_len = 0;
	int temp = 0;
	char *str;
	...
	if (match) {
		for (str = match; *str != ','; ++str) {
			str_len++;
		}
		temp = strlen("charge_miscdata_migic_number=");
		str_len -= temp;
		memcpy(result, (match + temp), str_len);
		index = atoi(result);
		dev_info(data->dev, "%s:line%d: magic number = 0x%x, result = %s\n", __func__, __LINE__, index, result);
		if (index != 0x5A) {
			dev_info(data->dev, "%s:line%d: magic number is not correct, index = 0x%x\n", __func__, __LINE__, index);
			return -EINVAL;
		}
	}
	...
	str_len = 0;
	memset(result, '\0', sizeof(result));
	match = strstr(saved_command_line, "charge_cycle_cnt=");
	if (match) {
		for (str = match; *str != ','; ++str) {
			str_len++;
		}
		temp = strlen("charge_cycle_cnt=");
		str_len -= temp;
		memcpy(result, (match + temp), str_len);
		index = atoi(result);
		data->charge_cycle_cnt = index;
		dev_err(data->dev, "%s:line%d: charge_cycle_cnt = %d, result = %s\n", __func__, __LINE__, index, result);
	}

	match = NULL;
	str_len = 0;
	memset(result, '\0', sizeof(result));
	match = strstr(saved_command_line, "charge_accumulate_percent=");
	if (match) {
		for (str = match; *str != ' '; ++str) {
			str_len++;
		}
		temp = strlen("charge_accumulate_percent=");
		str_len -= temp;
		memcpy(result, (match + temp), str_len);
		index = atoi(result);
		data->charge_accumulate_cnt = index;
		dev_err(data->dev, "%s:line%d: charge_accumulate_cnt = %d, result = %s\n", __func__, __LINE__, index, result);
	}
	return 0;
};

static int select_batt_info(struct power_supply *psy, int *num)
{
	int i = 0, len;
	struct sc27xx_fgu_data *data = power_supply_get_drvdata(psy);

	*num = 0;
	len = sizeof(batt_info_check_cycle) / sizeof(int);

	for(i = 0; i < len; i++){
		if(data->charge_cycle_cnt < batt_info_check_cycle[i]){
			break;
		}
		*num = i + 1;;
	}

	dev_err(data->dev, "wt charge_cycle_cnt=%d battery info num=%d\n",data->charge_cycle_cnt, *num);
	return 0;
}
//-bug696110,tankaikun.wt,add,2022/08/08,add bat protect
...
//+bug696110,tankaikun.wt,add,2022/08/08,add bat protect
static int sc27xx_fgu_batt_cycle_hw_init(struct sc27xx_fgu_data *data,
		   const struct sc27xx_fgu_variant_data *pdata)
{
	...
	//+bug696110,tankaikun.wt,add,2022/08/08,add batt protect
	ret = select_batt_info(data->battery, &num);
	if (ret) {
	 dev_err(data->dev, "failed to get battery id\n");
	 return ret;
	}
	data->batt_info_num = num;
	//-bug696110,tankaikun.wt,add,2022/08/08,add batt protect
...
static int sc27xx_fgu_batt_cycle_monitor(struct sc27xx_fgu_data *data)
{
	int ret=0;
	int num;
	...
	if (num != data->batt_info_num) {
		dev_err(data->dev, "update battery cycle & fg hw init\n");
		ret = sc27xx_fgu_batt_cycle_hw_init(data, data->pdata);
		if (ret) {
			dev_err(data->dev, "battery cycle failed to initialize fgu hardware\n");
		}
	}
	return ret;
}
...
static void sc27xx_fgu_track_check_work(struct work_struct *work)
{
	...
	printc("#batfull: vbat_avg=%d, ibat=(%d,%d), end_vol=%d, end_cur=%d, finish_cnt=%d\n",
		vbat_avg_mv,ibat_avg_ma,ibat_now_ma,data->track.end_vol,data->track.end_cur,data->track.track_finish_cnt);
	if (data->track.track_finish_cnt > SC27XX_FGU_TRACK_FINISH_CNT_THRESHOLD) {
		...
		total_mah = data->total_mah;
		printc("#batfull: end ibat_avg_ma = %d, vbat_avg_mv = %d\n",
		...
		*/
		...
		printc("#batfull: clbcnt = %d, start_clbcnt = %d, delta_mah = %d, "
		...
		if (abs(end_mah - total_mah) < total_mah / 5) {
			...
			printc("#batfull: track capacity is done end_mah = %d, diff_mah = %d\n",
				 end_mah, (end_mah - total_mah));
		} else {
			data->track.state = CAP_TRACK_IDLE;
			printc("#batfull: less than half standard capacity.\n");
		}
		return;
	}
...
static void sc27xx_fgu_track_capacity_monitor(struct sc27xx_fgu_data *data)
{
	...
	case CAP_TRACK_UPDATING:
		...
		if (vbat_avg_mv > data->track.end_vol &&
			...
			printc("#batfull: the capacity tracking finish condition is met!!!\n");
...
static void sc27xx_fgu_monitor(struct work_struct *work)
{
	...
	sc27xx_fgu_batt_cycle_monitor(data); //+bug696110,tankaikun.wt,add,2022/08/08,add bat protect
...
static int sc27xx_fgu_probe(struct platform_device *pdev)
{
	...
	mutex_init(&data->file_lock); //bug696110,tankaikun.wt,add,2022/08/08,add bat protect
	mutex_lock(&data->lock);
	...
	//+bug696110,tankaikun.wt,add,2022/08/08,add bat protect
	INIT_DELAYED_WORK(&data->file_data_work, sc27xx_fgu_file_data_work);
	get_cycle_cnt(data);
	//-bug696110,tankaikun.wt,add,2022/08/08,add bat protect
...
static void sc27xx_fgu_shutdown(struct platform_device *pdev)
{
	...
	data->file_buf[0] = data->charge_accumulate_cnt | 0x5A000000;// add magic number
	data->file_buf[1] = data->charge_cycle_cnt;
	dev_info(data->dev, "%s:line%d: buf[0]= 0x%x, buf[1] = 0x%x\n",
		__func__, __LINE__, data->file_buf[0], data->file_buf[1]);
	data->file_offset = 4;
	data->file_size = 8;
	data->write_file = true;
	schedule_delayed_work(&data->file_data_work, 0);
...
static struct platform_driver sc27xx_fgu_driver = {
	.shutdown = sc27xx_fgu_shutdown, //bug696110,tankaikun.wt,add,2022/08/08,add bat protect
