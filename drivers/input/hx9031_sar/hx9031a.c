...
#define HX9031A_HW_MONITOR_EN 0    //disable HW monitor by default
#define HX9031A_ALG_COMPILE_EN 1   //enable ALG
#define HX9031A_DRIVER_VER "Change-Id 000005"  //bug696976,fanwenming.wt,modify,20211215,modify HX9031A_DRIVER_VER
//#define USE_USB_CHANGE
...
static int8_t g_sar_init_complete = 0;// bug696976, fanwenming.wt, add, 20211117, add global variable g_sar_init_complete
static long wake_ms = 1000;
...
typedef struct {
    uint8_t Cmd;//0
    uint8_t Length; //1
    uint16_t HandleID; //2,3
    union { //4 bytes
        uint8_t udata[4];//4 5 6 7
        struct {
            int8_t status;
            int8_t type;
        };
...
}mSenMsgTpB_t, *pmSenMsgTp;

//+++bug696976,fanwenming.wt,modify,20211215,modify the capacity parameter of sar sensor
//hx9023E
static struct hx9031a_near_far_threshold hx9031a_ch_thres_default_hx9023e[HX9031A_CH_NUM] = {
    {.thr_near = 128, .thr_far = 128}, //ch0
...
static int hx9031a_id_check(void)
{
    ...
    for(ii = 0; ii < HX9031A_ID_CHECK_COUNT; ii++) {
        ...
        device_id = rxbuf[0];
        if((HX90XX_CHIP_ID_9031A == device_id || HX90XX_CHIP_ID_9031A1 == device_id))
            break;
        else
            continue;
    }
...
static void hx9031a_ch_cfg(uint8_t chip_select)
{
    ...
    //TODO：客户配置：每个通道的正负极都可以连接到任何一个CS。按需配置。
    if(HX9023E_ON_BOARD == chip_select) {
        CH0_POS = CS1;	//CS1 ANT
        CH0_NEG = NA;	//CS0 REF
        CH1_POS = NA;
        ...
        CH0_POS = CS1;  //grip_sensor
        CH0_NEG = CS0;  //grip_sensor ref
        CH1_POS = CS2;  //grip_sensor_sub
        CH1_NEG = NA;
        CH2_POS = CS3;  //grip_sensor_wifi
        CH2_NEG = CS4;  //grip_sensor_wifi ref
        CH3_POS = NA;
        CH3_NEG = NA;
...
static int16_t hx9031a_get_thres_near(uint8_t ch)
{
    ...
    ret = hx9031a_read(RA_PROX_HIGH_DIFF_CFG0_0x80 + (ch * CH_DATA_1BYTES), buf, 1);
    if(0 != ret) {
        PRINT_ERR("hx9031a_read failed\n");
    }

    hx9031a_ch_thres[ch].thr_near = buf[0] * 32;
...
static int16_t hx9031a_get_thres_far(uint8_t ch)
{
    ...
    ret = hx9031a_read(RA_PROX_LOW_DIFF_CFG0_0x85 + (ch * CH_DATA_1BYTES), buf, 1);
    if(0 != ret) {
        PRINT_ERR("hx9031a_read failed\n");
    }

    hx9031a_ch_thres[ch].thr_far = buf[0] * 32;
...
static int16_t hx9031a_set_thres_near(uint8_t ch, int16_t val)
{
    ...
    ret = hx9031a_write(RA_PROX_HIGH_DIFF_CFG0_0x80 + (ch * CH_DATA_1BYTES), buf, 1);//10位！！！
    if(0 != ret) {
        PRINT_ERR("hx9031a_write failed\n");
    }
...
static int16_t hx9031a_set_thres_far(uint8_t ch, int16_t val)
{
    ...
    ret = hx9031a_write(RA_PROX_LOW_DIFF_CFG0_0x85 + (ch * CH_DATA_1BYTES), buf, 1);//10位！！！
    if(0 != ret) {
        PRINT_ERR("hx9031a_write failed\n");
    }
...
static void hx9031a_get_prox_state(int16_t *data_compare, uint8_t ch_num)
{
    ...
    hx9031a_pdata.prox_state_reg_pre = hx9031a_pdata.prox_state_reg;//last state
    hx9031a_pdata.prox_state_cmp_pre = hx9031a_pdata.prox_state_cmp;//last state
    //====================================================================
...
static void hx9031a_sample(void)//输出raw bl diff lp offse五组数据，默认diff和raw (diff=lp-bl)
{
    ...
    bytes_all_channels = HX9031A_CH_NUM * bytes_per_channel;
    ret = hx9031a_read(RA_BL_OUT_CH0_0_0x90, rx_buf, bytes_all_channels);
    if(0 == ret) {
        for(ii = 0; ii < HX9031A_CH_NUM; ii++) {
    ...
    bytes_per_channel = CH_DATA_3BYTES;
    bytes_all_channels = HX9031A_CH_NUM * bytes_per_channel;
    ret = hx9031a_read(RA_LP_OUT_CH0_0_0xa0, rx_buf, bytes_all_channels);
    if(0 == ret) {
        for(ii = 0; ii < HX9031A_CH_NUM; ii++) {
...
static inline void sar_report_distance(struct input_dev *dev, unsigned int code, int value)
{
    mSenMsgTpB_t event;
    ...
    LOG_INFO("sar_report abs %d\n", value);
    if (g_sensor && value >= 0) {
        //memset(data, 0x00, 40);
        memset(&event, 0x00, sizeof(mSenMsgTpB_t));
        event.Cmd = 129;
        event.Length = sizeof(mSenMsgTpB_t);
        if(!strncmp(dev->name,"grip_sensor",sizeof("grip_sensor")))
            event.HandleID = 81;
        if(!strncmp(dev->name,"grip_sensor_sub",sizeof("grip_sensor_sub")))
            event.HandleID = 82;
        if(!strncmp(dev->name,"grip_sensor_wifi",sizeof("grip_sensor_wifi")))
            event.HandleID = 83;
        ...
        //P220721-03547,Modify,wt.linliwei,20220727,fix timestamp
        event.timestamp = div64_s64(timespec_to_ns(&tp), 1000);
        mutex_lock(&g_sensor->mutex_send);
        //iio_push_to_buffers(g_sensor->indio_dev, data);
        iio_push_to_buffers(g_sensor->indio_dev, (u8*)&event);
        mutex_unlock(&g_sensor->mutex_send);
        //LOG_INFO("sar_report abs id %d %lld\n", event.HandleID, event.timestamp);
    }
}
...
static void hx9031a_input_report(void)
{
    ...
    if (this->hx_first_boot)
	{
        if (true == hx9031a_channels[0].enabled)
		    ch_result[0] = data_offset_dac[0];
        if (true == hx9031a_channels[1].enabled)
		    ch_result[1] = data_offset_dac[1];
        if (true == hx9031a_channels[2].enabled)
		    ch_result[2] = data_offset_dac[2];

	    LOG_INFO("chx_back_cap %d, %d, %d\n", this->back_cap[0],this->back_cap[1],this->back_cap[2]);
	    LOG_INFO("chx_result %d, %d, %d\n", ch_result[0],ch_result[1],ch_result[2]);
	    LOG_INFO("interrupt_count is %d, %d\n", this->interrupt_count, this->ps_is_present);

	    if ((this->interrupt_count <= MAX_INT_COUNT) &&
                (ch_result[0] >= this->back_cap[0]) && (ch_result[1] >= this->back_cap[1]) && (ch_result[2] >= this->back_cap[2]))
	    {
		    ...
		    for (ii = 0; ii < 3; ii++) {
                ...
                sar_report_distance(hx9031a_channels[ii].hx9031a_input_dev, ABS_DISTANCE, 0);
            }
		    return;
	    }
    ...
    for (ii = 0; ii < num_channels; ii++) {
        ...
        if (BODYACTIVE == touch_state) {
...
#else
                if (wake_ms) wake_lock_timeout(&hx9031a_wake_lock, HZ * wake_ms/1000);
#endif
                input_report_abs(input, ABS_DISTANCE, 0);
                input_sync(input);
                sar_report_distance(input, ABS_DISTANCE, 0);
            ...
            if (channel_p->state == IDLE)
                ...
                input_sync(input);
                sar_report_distance(input, ABS_DISTANCE, 5);
...
/* DTS属性：
"tyhx,power-supply-type" //可选，如何供电，如果有的话，客户自行移植
...
static int hx9031a_parse_dt(struct device *dev)
{
    ...
    // for ss,libo7.wt,add,20210824
    hx9031a_pdata.grip_sensor_ch = 0;
    hx9031a_pdata.grip_sensor_sub_ch = 0;
    hx9031a_pdata.grip_sensor_wifi_ch = 0;
    of_property_read_u32(dt_node, "grip-sensor", &hx9031a_pdata.grip_sensor_ch);
	of_property_read_u32(dt_node, "grip-sensor-sub", &hx9031a_pdata.grip_sensor_sub_ch);
	of_property_read_u32(dt_node, "grip-sensor-wifi", &hx9031a_pdata.grip_sensor_wifi_ch);
...
static int hx9031_ps_get_state(struct power_supply *psy, bool *present)
{
	...
	retval = power_supply_get_property(psy, POWER_SUPPLY_PROP_ONLINE,
			&pval);
	if (retval) {
		LOG_INFO("%s psy get property failed", psy->desc->name);
		return retval;
	}
	*present = (pval.intval != 1) ? true : false;
	//LOG_INFO("%s is %s", psy->desc->name,
	//		(*present) ? "present" : "not present");
	return 0;
}

static int hx9031_ps_notify_callback(struct notifier_block *self,
		unsigned long event, void *p)
{
	...
	//LOG_INFO("psy->desc->name is %s",psy->desc->name);
	if (event == PSY_EVENT_PROP_CHANGED
			&& psy && psy->desc->get_property && psy->desc->name &&
			!strncmp(psy->desc->name, USB_POWER_SUPPLY_NAME, sizeof(USB_POWER_SUPPLY_NAME))){
		//LOG_INFO("ps notification: event = %lu", event);
		retval = hx9031_ps_get_state(psy, &present);
		if (retval) {
			LOG_INFO("psy get property failed");
			return retval;
		}
		if (event == PSY_EVENT_PROP_CHANGED) {
		...
		LOG_INFO("ps_is_present = %d\n", hx9031a_pdata.ps_is_present);
		if (hx9031a_pdata.hx_first_boot && hx9031a_pdata.ps_is_present && ((true == hx9031a_channels[0].enabled) || (true == hx9031a_channels[1].enabled) || (true == hx9031a_channels[2].enabled)))
        {
...
static int hx9031a_set_enable(struct sensors_classdev *sensors_cdev, unsigned int enable)
{
    ...
    for (ii = 0; ii < ARRAY_SIZE(hx9031a_channels); ii++) {
        if (hx9031a_channels[ii].used == true) {
            if (strcmp(sensors_cdev->name, hx9031a_channels[ii].hx9031a_sensors_classdev.name) == 0) {
                if (1 == enable) {
                    ...
                    hx9031a_channels[ii].state = PROXACTIVE;//IDLE;
                    ...
                    else
                    {
                        ...
                        input_sync(hx9031a_channels[ii].hx9031a_input_dev);
                        sar_report_distance(hx9031a_channels[ii].hx9031a_input_dev, ABS_DISTANCE, -1);
#if defined(CONFIG_SENSORS)
                    }
#endif
                } else {
...
//libo.7 for shub_core.c use
int capsensor_set_enable_ext(int id, int enable)
{
    int ret = -1;
    //+ bug696976, fanwenming.wt, add, 20211117, check whether the probe function is running successfully
    if(g_sar_init_complete != 1) {
        PRINT_ERR("g_sar_init_complete failed\n");
        return 0;
    }
    //- bug696976, fanwenming.wt, add, 20211117, check whether the probe function is running successfully
    mutex_lock(&hx9031a_ch_en_mutex);
    ...
    switch (id){
        case 81:
            if (enable == 1){
                hx9031a_channels[0].state = PROXACTIVE;//IDLE;
                ...
                //sar_report_distance(hx9031a_channels[0].hx9031a_input_dev, ABS_DISTANCE, 5);
                ...
                input_sync(hx9031a_channels[0].hx9031a_input_dev);
                sar_report_distance(hx9031a_channels[0].hx9031a_input_dev, ABS_DISTANCE, -1);
        ...
        case 82:
            if (enable == 1){
                hx9031a_channels[1].state = PROXACTIVE;//IDLE;
                ...
                //sar_report_distance(hx9031a_channels[1].hx9031a_input_dev, ABS_DISTANCE, 5);
                ...
                input_sync(hx9031a_channels[1].hx9031a_input_dev);
                sar_report_distance(hx9031a_channels[1].hx9031a_input_dev, ABS_DISTANCE, -1);
        ...
        case 83:
            if (enable == 1){
                hx9031a_channels[2].state = PROXACTIVE;//IDLE;
                ...
                //sar_report_distance(hx9031a_channels[2].hx9031a_input_dev, ABS_DISTANCE, 5);
                ...
                input_sync(hx9031a_channels[2].hx9031a_input_dev);
                sar_report_distance(hx9031a_channels[2].hx9031a_input_dev, ABS_DISTANCE, -1);
				       ...
				       char *buf)
{
    ...
    if(!strncmp(temp_input_dev->name,"grip_sensor",sizeof("grip_sensor"))){
        if(hx9031a_channels[0].enabled == true){
            status = 1;
        }else{
            status = 0;
        }
    ...
    if(!strncmp(temp_input_dev->name,"grip_sensor_sub",sizeof("grip_sensor_sub"))){
        if(hx9031a_channels[1].enabled == true){
            status = 1;
        }else{
            status = 0;
        }
    ...
    if(!strncmp(temp_input_dev->name,"grip_sensor_wifi",sizeof("grip_sensor_wifi"))){
        if(hx9031a_channels[2].enabled == true){
            status = 1;
        }else{
            status = 0;
        }
...
static void hx9031a_report_ch0_work_func(struct work_struct *work)
{
    ...
    hx9031a_pdata.back_cap[0] = ((rx_buf[1] << 8) | (rx_buf[0])) & 0x3FF;
    hx9031a_data_lock(HX9031A_DATA_UNLOCK);
    PRINT_INF("CH0 back_cap=%d\n", hx9031a_pdata.back_cap[0]);
...
static void hx9031a_report_ch1_work_func(struct work_struct *work)
{
    ...
    hx9031a_pdata.back_cap[1] = ((rx_buf[1] << 8) | (rx_buf[0])) & 0x3FF;
    hx9031a_data_lock(HX9031A_DATA_UNLOCK);
...
static void hx9031a_report_ch2_work_func(struct work_struct *work)
{
    ...
    hx9031a_pdata.back_cap[2] = ((rx_buf[1] << 8) | (rx_buf[0])) & 0x3FF;
    hx9031a_data_lock(HX9031A_DATA_UNLOCK);
...
static ssize_t hx9031a_wake_ms_show(struct class *class, struct class_attribute *attr, char *buf)
{
    PRINT_INF("wake_ms=%d\n", wake_ms);
    return sprintf(buf, "wake_ms=%d\n", wake_ms);
}

static ssize_t hx9031a_wake_ms_store(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
    int ret = -1;
    int value = 0;
...
static ssize_t hx9031a_offset_dac_show(struct class *class, struct class_attribute *attr, char *buf)
{
    ...
    for(ii = 0; ii < HX9031A_CH_NUM; ii++) {
        PRINT_INF("data_offset_dac[%d]=%dpF\n", ii, data_offset_dac[ii] * 234 / 1000);
        p += snprintf(p, PAGE_SIZE, "ch[%d]=%dpF ", ii, data_offset_dac[ii] * 234 / 1000);
    }
...
static struct class_attribute class_attr_wake_ms = __ATTR(wake_ms, 0664, hx9031a_wake_ms_show, hx9031a_wake_ms_store);
#if HX9031A_ALG_COMPILE_EN
...
//+++bug696976,fanwenming.wt,modify,20211215,modify sar sensor channel configuration
        ...
        char *buf)
{
    ...
    //CH0 background cap
    hx9031a_read(RA_OFFSET_DAC0_7_0_0x15, rx_buf, 2);
    cap_reg = ((rx_buf[1] << 8) | (rx_buf[0]));
    cap_reg = cap_reg & 0x3FF;//10位
    PRINT_ERR("CH0_background_cap=%d\n", cap_reg);
	len += snprintf(buf+len, PAGE_SIZE-len,
			"CH0_background_cap=%d;", cap_reg);

    //CH0 refer channel cap
    hx9031a_read(RA_OFFSET_DAC0_7_0_0x15, rx_buf, 2);
    cap_reg = ((rx_buf[1] << 8) | (rx_buf[0]));
    cap_reg = cap_reg & 0x3FF;//10位
    PRINT_ERR("CH0_refer_channel_cap=%d\n", cap_reg);
	len += snprintf(buf+len, PAGE_SIZE-len,
			"CH0_refer_channel_cap=%d;", cap_reg);

    //CH0 diff
    hx9031a_sample();
    len += snprintf(buf+len, PAGE_SIZE-len, "CH0_diff=%d",data_lp[0]-data_bl[0]);
        ...
        char *buf)
{
    ...
    cap_reg = ((rx_buf[1] << 8) | (rx_buf[0]));
    cap_reg = cap_reg & 0x3FF;//10位
    PRINT_ERR("CH2_background_cap=%d\n", cap_reg);
	len += snprintf(buf+len, PAGE_SIZE-len,
			"CH2_background_cap=%d;", cap_reg);

    //CH2 refer channel cap
    hx9031a_read(RA_OFFSET_DAC0_7_0_0x15 + 2, rx_buf, 2);
    cap_reg = ((rx_buf[1] << 8) | (rx_buf[0]));
    cap_reg = cap_reg & 0x3FF;//10位
    PRINT_ERR("CH2_refer_channel_cap=%d\n", cap_reg);
	len += snprintf(buf+len, PAGE_SIZE-len,
			"CH2_refer_channel_cap=%d;", cap_reg);

    //CH2 diff
    hx9031a_sample();
    len += snprintf(buf+len, PAGE_SIZE-len, "CH2_diff=%d",data_lp[1]-data_bl[1]);
        ...
        char *buf)
{
    ...
    //CH0 diff
    hx9031a_sample();
    len += snprintf(buf+len, PAGE_SIZE-len, "CH3_diff=%d",data_lp[2]-data_bl[2]);
...
//}end =============================================================
    ...
    device_create_file(hx9031a_channels[0].hx9031a_sensors_classdev.dev, &dev_attr_onoff);
#endif
	...
	/* +++add input_dev attribute for ss hal,libo7.wt */
		...
		if(!strncmp(hx9031a_channels[0].hx9031a_input_dev->name,"grip_sensor",sizeof("grip_sensor"))){
			device_create_file(&hx9031a_channels[0].hx9031a_input_dev->dev, &dev_attr_enable);
            PRINT_INF("grip_sensor attribute ENABLE has been created\n");
		}
		if(!strncmp(hx9031a_channels[1].hx9031a_input_dev->name,"grip_sensor_sub",sizeof("grip_sensor_sub"))){
			device_create_file(&hx9031a_channels[1].hx9031a_input_dev->dev, &dev_attr_enable);
            PRINT_INF("grip_sensor_sub attribute ENABLE has been created\n");
		}
		if(!strncmp(hx9031a_channels[2].hx9031a_input_dev->name,"grip_sensor_wifi",sizeof("grip_sensor_wifi"))){
			device_create_file(&hx9031a_channels[2].hx9031a_input_dev->dev, &dev_attr_enable);
            PRINT_INF("grip_sensor_wifi attribute ENABLE has been created\n");
		}
    ...
    if (ret < 0) {
        PRINT_ERR("Create ch0_cap_diff_dump file failed (%d)\n", &class_attr_ch0_cap_diff_dump);
        return ret;
    }

    ret = class_create_file(&capsense_class, &class_attr_ch2_cap_diff_dump);
    if (ret < 0) {
        PRINT_ERR("Create ch2_cap_diff_dump file failed (%d)\n", &class_attr_ch2_cap_diff_dump);
        return ret;
    }
    ...
    PRINT_INF("sensors_capsensor_ch0_cdev\n");
	sensors_capsensor_ch0_cdev.sensors_enable = hx9031a_set_enable;
    sensors_capsensor_ch0_cdev.sensors_poll_delay = NULL;
    ret = sensors_classdev_register(&hx9031a_channels[0].hx9031a_input_dev->dev, &sensors_capsensor_ch0_cdev);
    if (ret < 0)
		PRINT_ERR("failed to register ch0 sensors capsense device");

    PRINT_INF("sensors_capsensor_ch2_cdev\n");
    sensors_capsensor_ch2_cdev.sensors_enable = hx9031a_set_enable;
    sensors_capsensor_ch2_cdev.sensors_poll_delay = NULL;
    ret = sensors_classdev_register(&hx9031a_channels[1].hx9031a_input_dev->dev, &sensors_capsensor_ch2_cdev);
    if (ret < 0)
		PRINT_ERR("failed to register ch2 sensors capsense device");

    PRINT_INF("sensors_capsensor_ch3_cdev\n");
    sensors_capsensor_ch3_cdev.sensors_enable = hx9031a_set_enable;
    sensors_capsensor_ch3_cdev.sensors_poll_delay = NULL;
    ret = sensors_classdev_register(&hx9031a_channels[2].hx9031a_input_dev->dev, &sensors_capsensor_ch3_cdev);
    if (ret < 0)
		PRINT_ERR("failed to register ch3 sensors capsense device");
#endif
		hx9031a_pdata.hx_first_boot = true;
#ifdef USE_USB_CHANGE
		hx9031a_pdata.ps_notif.notifier_call = hx9031_ps_notify_callback;
...
#endif
    hardwareinfo_set_prop(HARDWARE_SAR,"HX9031,FW:V0.1");  // add hardware info for MMI
    g_sar_init_complete = 1;// bug696976, fanwenming.wt, add, 20211117, add flag
